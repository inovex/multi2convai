...
# and `pip install multi2convai` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "multi2convai", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n multi2convai -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "multi2convai", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "multi2convai", "YOUR_CONSOLE_SCRIPT"]
